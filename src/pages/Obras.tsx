...
import { WHATSAPP_GROUP_OBRAS, trackLeadObras } from "@/lib/lead-openai";

/**
 * ============================================================================
 * /obras — A GÊMEA DA /construcao, MEDINDO NA OPENAI (06/09/2026)
 * ============================================================================
 *
 * ⚠️ ESTA PÁGINA É CÓPIA FIEL DA `src/pages/Construcao.tsx`, e é para
 * continuar sendo: ele pediu "a aparência exatamente igual". A única coisa que
 * muda é quem mede — lá é o pixel da Meta, aqui é o do OpenAI Ads. Mesmo
 * grupo de WhatsApp, mesmos textos, mesmas medidas, mesmo desenho.
 *
 * 📌 MEXEU NUMA, MEXER NA OUTRA. Um teste em src/test/pixels.test.ts renderiza
 * as duas e compara o HTML caractere a caractere: se divergirem, ele quebra.
 * Preferir isso a fazer as duas páginas importarem um componente comum tem um
 * motivo medido — compartilhar código faria o Rollup criar um pedaço novo e
 * mudar o grafo de chunks da /construcao, que já está no ar. Umas centenas de
 * bytes duplicados compram a garantia de não encostar na página que existe.
 *
 * POR QUE DUAS PÁGINAS, e não uma com os dois pixels: um documento só pode ter
 * um `init` de cada plataforma, e misturar Meta e OpenAI na mesma página faria
 * cada campanha contar a conversão da outra. Página por plataforma é o que
 * mantém os dois painéis dizendo a verdade. Ver PIXEL_OPENAI_DA_ROTA em
 * rotas.ts e o bloco em scripts/pixel-openai.mjs.
 *
 ...
 * nenhuma medida cresce em tela grande, a página só fica centralizada.
 *
 * TODA MEDIDA E TODA COR VIERAM DO MOCKUP INTERATIVO que ele fechou em
 * 03/09/2026 para a /construcao, nestes valores exatos:
 *
...
const TEXTO = {
  ...
  // 🔴 O NÚMERO AQUI É INVENTADO — o mesmo da /construcao, aprovado por ele em
  // 03/09/2026 sabendo disso. Se um dia trocar pelo número real, trocar NAS
  // DUAS: o teste que compara as páginas quebra se só uma mudar.
 ...
 * o que sai daqui.
 *
 * ⚠️ É O ÚNICO PONTO EM QUE ESTA PÁGINA DIVERGE DA /construcao DE PROPÓSITO, e
 * a razão é o `noindex`: as duas são a mesma página e, indexáveis as duas,
 * disputariam a mesma busca — o mesmo estrago que o `noindex` da /ofertas
 * evita. Quem fica com o orgânico é a /construcao, que nasceu primeiro; esta
 * aqui vive de clique pago da OpenAI e não precisa de posição nenhuma.
 *
 * A canonical aponta para ELA MESMA, e não para a /construcao. Canonical
 * cruzada junto de `noindex` manda dois sinais que se contradizem (uma diz
 * "não me indexe", a outra diz "indexe aquela no meu lugar") e o Google trata
 * o par como ambíguo. Com `noindex` sozinho o recado é um só.
 */
const ObrasHead = () => (
  <Helmet>
    ...
    <link rel="canonical" href="https://apromofy.online/obras" />
    <meta property="og:type" content="website" />
    <meta property="og:url" content="https://apromofy.online/obras" />
    <meta
      property="og:title"
 ...
 *
 * A classe é a MESMA da /construcao (`construcao-clara`) de propósito: são duas
 * rotas de um desenho só, e uma classe gêmea seria uma regra a mais na folha,
 * que viaja embutida em todas as páginas do site.
 */
const useBodyAreia = () => {
  useEffect(() => {
    ...
    <>
      ...
      >
        ...
        <main className="mx-auto flex w-full max-w-[400px] flex-col items-center text-center">
          ...
          <a
            href={WHATSAPP_GROUP_OBRAS}
            target="_blank"
            rel="noopener noreferrer"
            onClick={(e) => trackLeadObras(e, WHATSAPP_GROUP_OBRAS)}
